...
var data = {
  ...
  tracks: [createTrack("#997FFF", note(audio, 880)),
           createTrack("#7FFFF0", note(audio, 659)),
           createTrack("#FFCC7F", note(audio, 587)),
           createTrack("#FF7F7F", note(audio, 523)),
           createTrack("#FFFA7F", note(audio, 440)),
           createTrack("#7FBAFF", kick(audio))]
};
...
(function draw() {
  ...
  drawButton(screen, data.step, data.tracks.length, "#7FEEFF");
...
function note(audio, frequency) {
  return function() {
    ...
    chain([
      ...
      // effect.
      createAmplifier(audio, 0.6, duration),
...
function buttonPosition(column, row) {
  return {
    x: BUTTON_SIZE / 3 + column * BUTTON_SIZE * 1.5,
    y: BUTTON_SIZE / 3 + row * BUTTON_SIZE * 1.5
  };
};
